...
{
//	dbg_str("sml_recognition_run_batch\n");
	...
	{
	#ifdef SML_USE_TEST_DATA
	dbg_str("sml_recognition_run_batch10\n");
...
//	dbg_str_hex32("sml_recognition_run_batch20", sensor_id);
	...
	switch(sensor_id)
	{
		...
		// tomo
					ret = kb_run_model((SENSOR_DATA_T *)data, num_sensors, KB_MODEL_mypipeline_rank_0_INDEX);
		if (ret >= 0){
			sml_output_results(KB_MODEL_mypipeline_rank_0_INDEX, ret);
			kb_reset_model(0);
		};
...
{
	dbg_str("sml_recognition_run_single\n");
